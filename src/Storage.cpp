...
void prefsSave(uint32_t items)
{
  if (items & SAVE_SETTINGS)
  {
    ...
    prefs.putString("latitude", validLatitude);
    prefs.putString("longitude", validLongitude);
    prefs.putString("unit", validUnit);
...
bool prefsLoad(uint32_t items)
{
  if (items & SAVE_SETTINGS)
  {
    ...
    analogWrite(PIN_LCD_BL, currentBrt);
    selectBand(bandIdx);
    delay(50);
    rx.setVolume(volume);
  }
