  ...
  if (netMode > NET_AP_ONLY && wifiConnect())
  {
    ...
    for (int j = 0; j < 10; j++)
    {
      if (ntpSyncTime())
      {
        ...
        {
          Serial.println("⚠️ Full time sync failed. Skipping fetch.");
        }
        break;
      }
      else
...
void webSetConfig(AsyncWebServerRequest *request)
{
  ...
  {
    validLatitude = request->getParam("latitude", true)->value();
    prefsSave |= SAVE_SETTINGS;
  }
  ...
  {
    validLongitude = request->getParam("longitude", true)->value();
    prefsSave |= SAVE_SETTINGS;
  }
  ...
  {
    validUnit = request->getParam("unit", true)->value();
    prefsSave |= SAVE_SETTINGS;
  }
...
const String webConfigPage()
{
  ...
  validUnit = prefs.getString("unit", "C"); // หรือ "metric"/"imperial" ตามที่คุณใช้
  prefs.end();
  ...
  return webPage(
      ...
      "<TR><TD CLASS='LABEL'>Username</TD><TD>" +
      webInputField("username", loginUsername) + "</TD></TR>"
                                                 "<TR><TD CLASS='LABEL'>Password</TD><TD>" +
      webInputField("password", loginPassword, true) + "</TD></TR>"
      ...
      webInputField("wifissid1", ssid1) + "</TD></TR>"
                                          "<TR><TD CLASS='LABEL'>Password</TD><TD>" +
      webInputField("wifipass1", pass1, true) + "</TD></TR>"

                                                // Location
                                                "<TR><TH COLSPAN=2 CLASS='HEADING'>Location Settings</TH></TR>"
                                                "<TR><TD CLASS='LABEL'>Latitude</TD><TD>" +
      webInputField("latitude", validLatitude) + "</TD></TR>"
                                                 "<TR><TD CLASS='LABEL'>Longitude</TD><TD>" +
      webInputField("longitude", validLongitude) + "</TD></TR>"
      ...
      (validUnit == "F" ? " SELECTED" : "") + ">Fahrenheit</OPTION>"
                                              ...
                                              "<TR><TD CLASS='LABEL'>SSID</TD><TD>" +
      webInputField("wifissid2", ssid2) + "</TD></TR>"
                                          "<TR><TD CLASS='LABEL'>Password</TD><TD>" +
      webInputField("wifipass2", pass2, true) + "</TD></TR>"

                                                // WiFi 3
                                                "<TR><TH COLSPAN=2 CLASS='HEADING'>WiFi Network 3</TH></TR>"
                                                "<TR><TD CLASS='LABEL'>SSID</TD><TD>" +
      webInputField("wifissid3", ssid3) + "</TD></TR>"
                                          "<TR><TD CLASS='LABEL'>Password</TD><TD>" +
      webInputField("wifipass3", pass3, true) + "</TD></TR>"
