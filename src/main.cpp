...
//  Remote & RDG Led setup
  ...
  if (!prefsLoad(SAVE_MEMORIES | SAVE_VERIFY))
    prefsSave(SAVE_MEMORIES);

  // If loading bands fails, save default bands
  if (!prefsLoad(SAVE_BANDS | SAVE_VERIFY))
...
extern bool tuneToMemory(const Memory *);

void recallMemorySlot(uint8_t idx)
{
  if (idx < MEMORY_COUNT && memories[idx].freq)
  {
    tuneToMemory(&memories[idx]);
    Serial.printf("📻 Recall memory %d: %d kHz\n", idx, memories[idx].freq);
    drawScreen(); // อัปเดตหน้าจอ
  }
  else
  {
    Serial.printf("❌ Memory %d empty\n", idx);
  }
}
